...
// i 表示一个6位的二进制数，每一位代表一个颜色分量的值
// 每一位的取值范围是0或1
// 第1位和第3位，作为红色分量的值
// 第2位和第4位，作为绿色分量的值
// 第0位和第5位，作为蓝色分量的值
// 每个颜色分量的值的范围在 [0,63] 之间, 最后的+1操作，使其范围变更为 [1,64]
// 返回一个表示 RGBA 的值
...
// i 表示一个6位的二进制数，每一位代表一个颜色分量的值
// 这个函数是其同名函数的反向实现，目的是将 RGB 颜色的每个分量存储到 col 这个数组中
						  ...
						  float maxx, float maxy, float maxz, unsigned int col)
{
	...
	// 这里将圆柱体划分为16条线段表示，线段的数量会影响圆柱体的精度。
	// 数量过多会导致顶点数量增加，性能降低。
	// 数量过少，则有可能导致出现锯齿或不连续的情况，但性能会得到提升。
	static const int NUM_SEG = 16;
	// dir 存储的是顶点的位置，因为每条线段由两个点组成，因此需要乘以2
	static float dir[NUM_SEG*2];
	static bool init = false;
	// 初始化顶点，计算每个顶点的 x,y 坐标
	if (!init)
	{
		init = true;
	...
	// 计算圆柱体的中心点坐标，以及底部和顶部圆形的半径。
	const float cx = (maxx + minx)/2;
	const float cz = (maxz + minz)/2;
	const float rx = (maxx - minx)/2;
	const float rz = (maxz - minz)/2;
	
	// 将圆柱体的每条边绘制成线段，并将线段的顶点位置添加到 DebugDraw 对象中。
	// 对于每条边，计算其起点和终点的坐标，并将其作为参数调用 DebugDraw 对象
	// 的 vertex 方法。
	...
	// 将圆柱体的底部和顶部圆形绘制出来，并将顶点位置添加到 DebugDraw 对象中。
	// 计算每个点在圆柱体底部和顶部的坐标，每个4条线段绘制一个点。
