...
// 绘制单个导航网格（瓦片），包括多边形、多边形边界、导航路径连接线
						 ...
						 const dtMeshTile* tile, unsigned char flags)
{
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		unsigned int col;
		// 判断多边形是否在查询的关闭列表当中，设置为黄色半透明，加以区分
		if (query && query->isInClosedList(base | (dtPolyRef)i))
			col = duRGBA(255,196,0,64);
		else
		{
			// 如果设置了以指定颜色绘制的标志位，就以指定的颜色进行绘制
			// 否则，使用区域颜色进行绘制
		...
		for (int j = 0; j < pd->triCount; ++j)
		{
			...
			for (int k = 0; k < 3; ++k)
			{
				// 如果当前顶点是基本多边形上的顶点，则使用网格（瓦片）中的基本多边形顶点绘制
				// 否则，使用网格（瓦片）中的详细顶点绘制
				if (t[k] < p->vertCount)
	...
	if (flags & DU_DRAWNAVMESH_OFFMESHCONS)
	{
		...
		{
			const dtPoly* p = &tile->polys[i];
			// 如果多边形的类型，不是连接线类型，就跳过绘制
			if (p->getType() != DT_POLYTYPE_OFFMESH_CONNECTION)	// Skip regular polys.
				continue;
			
			unsigned int col, col2;
			// 判断多边形是否在查询的关闭列表当中，设置为黄色的透明色，加以区分
			if (query && query->isInClosedList(base | (dtPolyRef)i))
			...
			bool startSet = false;
			bool endSet = false;
			// 遍历连接线所在的多边形上的所有连接，检查起点和终点是否在边缘上存在连接
			for (unsigned int k = p->firstLink; k != DT_NULL_LINK; k = tile->links[k].next)
			{
