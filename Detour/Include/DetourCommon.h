...
/// 
/// 计算三角形 ABC 在 xz 平面上的有向面积，或者线段 AB 与 点 C 之间的关系
...
// 找到一个数，这个数是大于或等于给定的数字的最小的2的整数次幂
// 例如：2、4、6、8 ... 256
...
// 找到一个数，它是由2的多少次幂得到的。换句话说，就是找到2
// 的整数次幂，但是这个次幂的值不能大于参数 v。
// 例如：参数 v = 7，那么结果为2.因为2的3次幂为8，大于7.
...
// 函数从一个指针所指的内存位置获取特定类型的指针，然后，将向后移动一定的距离。
template<typename TypeToRetrieveAs>
