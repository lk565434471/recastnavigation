...
struct BVItem
{
	// bmin 和 bmax 定义了 AABB（包围盒）的最小和最大顶点坐标，用于碰撞检测
						...
						unsigned short* bmin, unsigned short* bmax)
{
	...
	// 遍历所有节点，以找到范围最大的 AABB（包围盒）碰撞检测体
...
// 这个函数的作用是，将 bvtree 的所有元素分割成多个较小的子空间，并构建一颗 bvtree。
static void subdivide(BVItem* items, int nitems, int imin, int imax, int& curNode, dtBVNode* nodes)
{
	...
	// 分裂，当叶子节点无法容纳足够多的对象时，即叶子节点存储的对象数量超过了预分配的阈值
	else
	{
		// Split
		...
		// 如果 iescape 取反后，是一个负数，表示这是一个逃逸节点。
		node.i = -iescape;
	}
}
...
static int createBVTree(dtNavMeshCreateParams* params, dtBVNode* nodes, int /*nnodes*/)
{
	...
	for (int i = 0; i < params->polyCount; i++)
	{
		...
		if (params->detailMeshes)
		{
			...
			// BV 树在所有维度上使用了坐标系统（coordinate system）
