...
dtStatus dtNavMeshQuery::init(const dtNavMesh* nav, const int maxNodes)
{
	...
	// 如果 m_nodePool 没有被初始化，或者其持有的节点数小于 maxNodes
	// 重新构造 m_nodePool
	if (!m_nodePool || m_nodePool->getMaxNodes() < maxNodes)
	{
		...
		// dtNextPow2(maxNodes/4) 计算的是哈希表的大小，其值是，最接近 maxNodes/4 的值，且是2的幂次方的整数
...
///
/// 使用细节多边形来查找表面高度的方法。如果对速度要求比较高，可以考虑使用 closestPointOnPolyBoundary 方法
/// 但准确性可能会有所降低，这取决于给定点的位置。
/// 
/// pos 可能不在多边形或导航网格的边界内部
...
/// 
/// 比 closestPointOnPoly 函数更快的方法
/// 
/// 如果提供的位置在多边形的 xz 平面边界内（上方或下方），那么参数 pos 和 closest 将相等。
/// 
/// closest 的高度将与多边形的边界高度一致，不考虑详细的高度信息。
/// 
/// pos 参数不必在多边形或导航网格的边界内。
...
/// of the polygon.
/// 
/// 如果给定的位置超出了多边形xz平面上的边界范围，则返回 DT_FAILURE | DT_INVALID_PARAM。
dtStatus dtNavMeshQuery::getPolyHeight(dtPolyRef ref, const float* pos, float* height) const
{
	...
	// 对于离散连接，以前的做法是返回成功状态，但是在DetourNavMesh中的
	// getPolyHeight方法并不这样处理，所以在这里对离散连接做了特殊处理
...
// 当中心点与最近点重合时，isOverPoly 参数被设置为 true；
// 这意味着指定的中心点位于多边形内部或边界上。然后，还存在一种特殊情况，即当爬升高度的值位于多边形内部时
// 也将被视为位于多边形内部。
dtStatus dtNavMeshQuery::findNearestPoly(const float* center, const float* halfExtents,
										 const dtQueryFilter* filter,
										 dtPolyRef* nearestRef, float* nearestPt, bool* isOverPoly) const
{
	...
	// 在调用 queryPolygons 函数时，如果之前的参数已经满足要求，那么该函数将继续检查剩下的参数。
										 ...
										 const dtQueryFilter* filter, dtPolyQuery* query) const
{
	...
	if (tile->bvTree)
	{
		...
		// 计算量化框（对边界框进行量化）
		// 量化的过程是将实际空间坐标映射到离散的网格行列位置的计算过程
	...
	else
	{
		...
		for (int i = 0; i < tile->header->polyCount; ++i)
		{
			...
			if (!filter->passFilter(ref, tile, p))
				continue;
			// Calc polygon bounds.
			// 计算多边形边界，遍历所有顶点，取一个最小和一个最大顶点作为检测边界
...
///
/// 在进行多边形查询时，会分批次执行。传递给查询函数的多边形的边界框与中心点和半边长相交。
/// dtPolyQuery::process 函数会被多次调用，直到所有的多边形完成相交处理。
dtStatus dtNavMeshQuery::queryPolygons(const float* center, const float* halfExtents,
									   const dtQueryFilter* filter, dtPolyQuery* query) const
{
	dtAssert(m_nav);
	...
	// 因为寻路算法是基于2d的，因此这里计算出起始的minx和miny（minx 是代表格子的列，miny 代表格子的行）
	// maxx 和 maxy 同理，这样就构造出所需遍历的格子范围
...
/// path query.
///
/// 警告：在调用 finalizeSlicedFindPath() 或 finalizeSlicedFindPathPartial() 函数之前
/// 不要调用非切片方法，否则可能导致数据损坏。
/// 
/// 在切片路径查询的过程中，查询所使用的过滤器 filter 指针会被保存，并在整个查询过程中被使用。
											...
											const dtQueryFilter* filter, const unsigned int options)
{
	...
	// 在路径规划和导航中，通过 DT_FINDPATH_ANY_ANGLE 选项可以在性能
	// 和质量之间进行平衡
	// 该选项通过射线投射来快速搜索路径，但仍然会考虑路径上的成本因素，以
	// 确保最终生成的路径有效
	if (options & DT_FINDPATH_ANY_ANGLE)
	{
		// limiting to several times the character radius yields nice results. It is not sensitive 
		// so it is enough to compute it from the first tile.
		// 限制路径规划时的搜索半径范围为角色半径的几倍，可以获得更好的效果。
		// 因此只计算第一个瓦片就足够了。
...
dtStatus dtNavMeshQuery::updateSlicedFindPath(const int maxIter, int* doneIters)
{
	...
	while (iter < maxIter && !m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			// The node is already in open list and the new result is worse, skip.
			// 如果一个节点已经在开放列表中，并且新的计算结果更差，就直接跳过。
			if ((neighbourNode->flags & DT_NODE_OPEN) && total >= neighbourNode->total)
				continue;
			// The node is already visited and process, and the new result is worse, skip.
			// 如果一个节点已经被访问并处理过，并且新的计算结果更差，直接跳过。
			if ((neighbourNode->flags & DT_NODE_CLOSED) && total >= neighbourNode->total)
				continue;
...
dtStatus dtNavMeshQuery::finalizeSlicedFindPath(dtPolyRef* path, int* pathCount, const int maxPath)
{
	...
	*pathCount = 0;
	...
	// 由于 initSlicedFindPath or updateSlicedFindPath 函数执行失败
	// 因此不需要处理了
...
/// position.
///
/// 该方法执行的是通常称为 "string pulling" 的操作。
/// 
/// 该方法会将起始位置固定在路径的第一个多边形上，并将结束位置固定在最后一个多边形上。
/// 因此，起始位置和结束位置通常应该在或非常靠近第一个和最后一个多边形。
/// 
/// 返回的多边形引用表示进入相关路径位置的多边形的引用标识符。与结束点相关联的引用标识符始
/// 终为零。这样可以将离散的路径点与它们所代表的多边形进行匹配，例如连接跨越非导航区域的连
/// 接点与其代表的多边形。
/// 
/// 如果提供的结果缓冲区对于整个结果集来说太小，它们将从起始位置向结束位置填充尽可能多的数据。
...
/// position.
///
/// 用于小范围位移和少量多边形的方法。如果用于距离过大的情况，结果集将形成一个不完整的路径。
/// 
/// resultPos 如果目标位置可到达，resultPos 等于 endPos。否则，resultPos 将返回最近可达位置。
/// 
/// resultPos 结果位置不会被投影到导航网格的表面上，如果需要投影，可以使用 getPolyHeight 方法。
/// 
/// 此方法以 raycast 相同的方法处理结果位置。（以2D的方式）有关详细信息，请参阅该方法的文档。
/// 
/// 如果 visited 数组的大小不足以容纳整个结果集，它将尽可能从起始位置向目标位置填充，直到数组已满。
/// 
	...
	*visitedCount = 0;
	...
	while (nstack)
	{
		...
		for (int i = 0, j = (int)curPoly->vertCount-1; i < (int)curPoly->vertCount; j = i++)
		{
			...
			// 这个标记表明，两个多边形不是相邻的，是经过优化的，即通过中间多边形的计算，可以认定为它们有相邻的关系
			// 例如：有 A、B、C 三个多边形，可以从 A 走到 C，期间经过 B，那么可以认为 A 与 C 相邻。
			...
			else
			{
				for (int k = 0; k < nneis; ++k)
				{
					...
					// 如果搜索路径与搜索约束之间的距离太远，则跳过这个连接
					// getPortalPoints 可以精准地计算出门户点，但在某些情况下可能会比较耗时。
					const float* vj = &verts[j*3];
					const float* vi = &verts[i*3];
					float tseg;
	...
	{
		// Reverse the path.
		// 这段代码的意思就是，从最优路径的节点开始，反向找到第一个被添加的节点路径
		dtNode* prev = 0;
		...
		// Store result
		// 记录所有经过的路径id到visited数组
		node = prev;
		do
		{
			visited[n++] = node->id;
			// 如果已经达到存储路径的最大上限，则标记下来，告诉外部的使用者这是一个不完整的路径
			// 基于这种结果，外部使用者可以先尝试移动到一个中间位置，然后再继续最终目标路径的搜索
			// 同时，也可以反复调用这个函数，直到得到一个完整的路径，这取决于具体的实现。
			if (n >= maxVisitedSize)
			{
				status |= DT_BUFFER_TOO_SMALL;
				break;
...
/// 这个方法用于快速的、短距离检测。
/// 
/// 如果路径数组太小，无法容纳所有结果，它将从起始位置向结束位置填充，直到填满。
/// 
/// 使用 RaycastHit 的 Hit 参数 t
/// 
/// 如果 Hit 参数是一个非常大的值（FLT_MAX），则表示射线击中了结束位置。在这种情况下，路径表
/// 示了通向结束位置的有效路径，而 hitNormal 的值是未定义的。
/// 
/// 如果 Hit 参数为零，则表示起始位置在被击中的墙上，此时 hitNormal 的值是未定义的。
/// 
/// 如果 0 < t < 1.0，则适用以下规则：
/// 
/// 
/// 用例限制
/// 
/// 射线检测忽略了结束位置的 y 值（2D 检测）。这对于它的使用有重要限制。例如：
/// 
/// 考虑一个场景，其中有一个主楼层和一个悬挂在主楼层上方的二楼阳台。因此，一楼网格
/// 延伸到阳台网格的下方。起始位置位于一楼的某个位置，结束位置位于阳台上。
/// 
/// 射线检测将沿着一楼网格向结束位置进行搜索。如果到达了结束位置的 xz 坐标，则射线检
/// 测会表示为 FLT_MAX（没有击中墙壁），表示到达了结束位置。这是为什么该方法适用于短距
/// 离检测的一个例子。
dtStatus dtNavMeshQuery::raycast(dtPolyRef startRef, const float* startPos, const float* endPos,
								 const dtQueryFilter* filter, const unsigned int options,
								 dtRaycastHit* hit, dtPolyRef prevRef) const
{
	...
	while (curRef)
	{
		...
		if (segMax == -1)
		{
			...
			// add the cost
			// 累计路径消耗，前一个网格和当前网格距离的平方 + 当前区域的消耗
...
/// be filled to capacity.
/// 
/// 这个方法针对小的搜索半径和少量的结果多边形进行了优化
/// 
/// 候选多边形是通过从起始多边形开始搜索导航图来找到的
/// 
/// 与 findPolysAroundCircle 方法相同，对于相交测试也有一些限制
/// 
/// 中心点的值被用作成本计算的起点。它没有投影道网格表面，所以它的y值会影响成本
/// 
/// 相交测试在二维空间中进行。所有多边形和搜索圆都被投影到xz平面上。因此，中心点的y值不会影响相交测试。
/// 
/// 如果结果数组的容量不足以容纳整个结果集，它们将填满到容量上限。
/// 
/// 该方法适用于小范围搜索和少量结果的情况。它在性能和内存占用方面进行了优化。注意，具体的搜索半径和结果多边形的
/// 数量应根据具体情况进行调整，以获得最佳性能和结果。
...
/// 
/// 如果提供了 segmentRefs 参数，则返回所有多边形的边界段。否则，只返回墙壁边界段。
/// 
/// 如果使用过滤器导致相邻多边形变为不可通过，则通常作为门户的边界段将被包含在结果集中，被视为墙壁。
/// 
/// segmentVerts 和 segmentRefs 缓冲区应根据导航网格中的每个多边形的最大边界段来调整大小。
/// 
											 ...
											 const int maxSegments) const
{
	...
	*segmentCount = 0;
	...
	for (int i = 0, j = (int)poly->vertCount-1; i < (int)poly->vertCount; j = i++)
	{
		...
		// Add sentinels
		// 添加哨兵。
		// 哨兵通常用于简化数据结构的处理，可以作为明确的边界和终止条件。
		...
		for (int k = 1; k < nints; ++k)
		{
			...
			// Wall segment.
			// 墙壁边缘段
			// 因为墙壁是两个门户之间的区域，因此通过前一个门户的终点和当前门户的起点，确定一条线段
			// 通过这样的处理，可以保证墙壁分割线段的起点和终点位置连续且不重叠，准确地定义了墙壁的形状。
...
bool dtNavMeshQuery::isValidPolyRef(dtPolyRef ref, const dtQueryFilter* filter) const
{
	const dtMeshTile* tile = 0;
	const dtPoly* poly = 0;
	// 通过多边形的复合id，找到对于的多边形对象和其所属的瓦片
	dtStatus status = m_nav->getTileAndPolyByRef(ref, &tile, &poly);
	// If cannot get polygon, assume it does not exists and boundary is invalid.
	// 如果没有找到多边形对象，则认为它不存在，且边界无效
	if (dtStatusFailed(status))
		return false;
	// If cannot pass filter, assume flags has changed and boundary is invalid.
	// 如果不能通过查询条件筛选器的验证，则认为标志位被修改，且边界无效。
