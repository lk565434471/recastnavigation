						 ...
						 const float px, const float py)
{
	...
	// 检查水平方向是否有重叠
	// 为了避免两个边界端点相接时，被认为时连接在一起的，需要在端点处流出一定
	// 的空隙。这个空隙可以通过将线段的长度略微缩小来实现。
	// 这里做的事情就是左上角向右缩进，左下角向左缩进
	// 然后，检查缩小后，是否有重叠
	/*
	*		  * 与 * 包围的这个区间被认为是允许的误差范围，或者，一个边界范围
	*         在这个范围内，则不认为是重叠的，因为他们有可能是拥有共享边，这被
	*         认为是相邻的，而非重叠。
	* 
	*         |----------------------------------|
	*         |                *    *            |
...
// 函数中的 slab，在计算机图形学和计算几何中，通常指代一个矩形（或其中的一个平面）
// 有两个端点，一个最小和最大的坐标。
...
// 取出一个空闲链接的索引，并重置空闲链接的索引为取出元素的下一个元素
...
void dtNavMesh::connectIntLinks(dtMeshTile* tile)
{
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		// 构建反向边缘链接，这样链接就会按照从到高索引的顺序出现在链表中。
		// 函数会遍历每个多边形的边缘，然后为每条边缘构建反向链接。
		for (int j = poly->vertCount-1; j >= 0; --j)
		{
			// Skip hard and non-internal edges.
			// 跳过 “硬边缘” 和 “非内部边缘”
			// “硬边缘” 就是无法在两个多边形之间穿越的边，可以理解为障碍物
			// “非内部边缘” 就是不属于两个多边形的共享边
			// 总的来说，就是无法穿越的边。
			if (poly->neis[j] == 0 || (poly->neis[j] & DT_EXT_LINK)) continue;

			// 取出一个空闲链接的索引，然后重置空闲链接的索引为取出元素的下一个元素的索引
			unsigned int idx = allocLink(tile);
			// 如果 idx == DT_NULL_LINK，说明没有可以使用的链接
			if (idx != DT_NULL_LINK)
			{
				dtLink* link = &tile->links[idx];
...
void dtNavMesh::baseOffMeshLinks(dtMeshTile* tile)
{
	...
	for (int i = 0; i < tile->header->offMeshConCount; ++i)
	{
		...
		// 半延伸，意思是以中心开始，沿着每个轴延伸半个长度。
		// 例如：halfExtents = [2, 1, 3]，那么它实际的大小为 [4, 2, 6]
...
bool dtNavMesh::getPolyHeight(const dtMeshTile* tile, const dtPoly* poly, const float* pos, float* height) const
{
	...
	// DT_VERTS_PER_POLYGON = 6，是因为使用的六边形 [(x, y, z)]
	...
	// ok.
	// 如果上面所有的三角形检查都失败（可能发生在存在退化三角形或较大浮点值的情况下），因此
	// 只需要选择最近的点。这种情况几乎不会发生，所以这里的额外迭代是可以接受的。
	float closest[3];
...
{
	float bmin[3], bmax[3];
	// 以 center 为中心，向两端颜色一个长度为 halfExtents 的线段
	...
	// 在附近的多边形中，找到距离点 nearestPt 最近的多边形
	dtPolyRef nearest = 0;
...
/// 
/// 如果给定的 tiles 数组大小不足以容纳整个结果集，这个函数不会失败。
/// 它将简单地将数组填充到容量上限。
...
/// @see #addTile
/// 
/// 这个函数返回一个网格数据，也就是说，在这之后，它可以被重新加入到导航网格中。
...
/// 
/// 离散连接点在导航网格中被存储为特殊的、只有一条边的2个顶点的多边形。这两个顶点中至少有
/// 一个被期望位于普通多边形内部。因此，离散连接点是从普通多边形的一个端点“进入”的。这个
/// 普通的多边形由prevRef参数标识。
dtStatus dtNavMesh::getOffMeshConnectionPolyEndPoints(dtPolyRef prevRef, dtPolyRef polyRef, float* startPos, float* endPos) const
{
	...
	// 从起点到终点的顺序传递
	// 从终点到起点的顺序传递
	// 下面定义的2个index变量的作用，就是这两种传递方式的实现
	int idx0 = 0, idx1 = 1;
