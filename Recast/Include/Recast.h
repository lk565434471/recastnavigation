...
// 这个类提供了一个可选的接口，用于记录和追踪 Recast 构建过程中的性能和日志信息。
// 这个类本身并不提供日志或计时器功能，必须由具体的实现通过重写受保护的成员函数来
// 提供。此外，这个类也不提供提取日志消息的接口（只能添加），因此具体实现必须提供
// 一个这样的接口。
// 如果不需要记录日志或计时器信息，只需通过这个类的实例传递 Recast 构建过程即可。
class rcContext
{
public:
	/// Constructor.
...
// 这是一个辅助类，用于在创建时，启动一个定时器，在退出作用域时，停止定时器。
class rcScopedTimer
{
public:
...
struct rcConfig
{
	...
	// 高度场周围不可行走的边界大小
	// 用于避免计算中的边缘错误，因为实际的地形或障碍物并不是精确的网格，所以需要在边缘处添加额外的边界（空间）用于检测。
	int borderSize;
	...
	// 可行走的最大坡度，在路径规划中，用于计数单元格是否可以通行
	// 这个值用于可移动物体的爬坡能力判断
	float walkableSlopeAngle;

	/// Minimum floor to 'ceiling' height that will still allow the floor area to 
	/// be considered walkable. [Limit: >= 3] [Units: vx]
	/// 描述的是地面与天花板高度之间的最小距离，当距离小于该值时，这个区域被标记为不可行走区域。
	/// 这个字段与可移动物体本身没有直接关联，但都是用于判断区域是否可行走的一部分 
	int walkableHeight;
	...
	/// 在生成可行走区域时，从障碍物边缘侵蚀/缩小可行走区域的距离。目的是为了防止路径搜索算法在
	/// 靠近障碍物边缘的位置时出现问题，因此需要将障碍物的边缘部分与可行走区域缩小一定的距离，这
	/// 个距离指的就是 “边缘侵蚀”，也是该字段的作用。
	int walkableRadius;
	
	/// The maximum allowed length for contour edges along the border of the mesh. [Limit: >=0] [Units: vx] 
	/// 边缘轮廓线的最大长度。边缘轮廓线是连接网格区域和非网格区域（如围墙或地图边界）的轮廓线。
	int maxEdgeLen;
	
	/// The maximum distance a simplified contour's border edges should deviate 
	/// the original raw contour. [Limit: >=0] [Units: vx]
	/// 简化的轮廓边界与原始轮廓边界的最大偏差距离。简化轮廓可以减少计算量，但同时也可能会
	/// 对精度造成影响。在这种情况下，通过控制最大偏差距离来平衡计算量和精度。
	float maxSimplificationError;
	
	/// The minimum number of cells allowed to form isolated island areas. [Limit: >=0] [Units: vx]
	/// 允许出现孤立区域的最小单元格数。这些单元格不属于可行走区域、障碍物区域。
	/// 这个最小值可以帮助排除由于高度场中存在的小坑、小洼地，导致寻路算法出现异常或过度消耗计算资源的情况。
	int minRegionArea;
	
	/// Any regions with a span count smaller than this value will, if possible, 
	/// be merged with larger regions. [Limit: >=0] [Units: vx] 
	/// 如果一个区域所包含的跨度（spans）数量小于这个值，且它与相邻的区域可以合并，那么它将
	/// 被合并到相邻的更大的区域中。这样做的目的是为了减少区域的数量，提高处理效率。
	int mergeRegionArea;
	
	/// The maximum number of vertices allowed for polygons generated during the 
	/// contour to polygon conversion process. [Limit: >= 3] 
	/// 生成的多边形所允许的最大顶点数.超过这个数量，会将其拆分成多个小的多边形，以满足这个限制。
	int maxVertsPerPoly;
	
	/// Sets the sampling distance to use when generating the detail mesh.
	/// (For height detail only.) [Limits: 0 or >= 0.9] [Units: wu] 
	/// 生成局部网格时使用的采样距离，指的是生成局部网格中相邻两个点之间的距离
	/// 距离越小，生成的局部网格越精细，但同时占用的内存也会变多
	float detailSampleDist;
	
	/// The maximum distance the detail mesh surface should deviate from heightfield
	/// data. (For height detail only.) [Limit: >=0] [Units: wu] 
	/// 局部网格表面可以偏离基础高度场数据的最大距离。这个值可以控制细节网格的准确度和复杂度。
	/// 如果该值太小，可能会导致细节网格不足以准确表示原始数据；
	/// 如果该值太大，可能会导致细节网格过于复杂，消耗过多的计算资源。
...
struct rcHeightfield
{
	...
	// 每个单元格的大小。（Cell Size，在xz平面上）
	float cs;			///< The size of each cell. (On the xz-plane.)
	// 每个单元格的高度。（Cell height，沿y轴的最小增量）
	float ch;			///< The height of each cell. (The minimum increment along the y-axis.)
	// 高度场的跨度数组，其大小为：width * height
	rcSpan** spans;		///< Heightfield of spans (width*height).
	// 跨度池的链表
	rcSpanPool* pools;	///< Linked list of span pools.
	// 下一个可用的跨度对象（即未被使用的 span 对象）
