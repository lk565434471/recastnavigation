											  ...
											  dtObstacleAvoidanceDebugData* debug)
{
	...
	for (int i = 0; i < m_nsegments; ++i)
	{
		...
		if (seg->touch)
		{
			...
			// 如果代理的速度指向线段，则不存在碰撞。就是说代理的运动方向与给定线段的方
			// 向相同或接近相同时，不存在碰撞的情况。
	...
	// 规范化侧向偏差，以放置其过于显著。意思是，确保偏差在决策过程中的
	// 影响不会占据主导地位。
	if (nside)
		side /= nside;
													 ...
													 dtObstacleAvoidanceDebugData* debug)
{
	...
	// 旋转采样点，根据余弦计算下一个点的x坐标
	const float ca = cosf(da);
	// 旋转采样点，根据正弦计算下一个点的y坐标
	...
	for (int j = 0; j < nr; ++j)
	{
		// 表示当前迭代与总迭代次数的比例关系，用于确定每个采样点的权重
		const float r = (float)(nr-j)/(float)nr;
		// 这里对j求余，是为了确保索引不会超出有效范围
		// 计算采样点的 x,y 坐标
		pat[npat*2+0] = ddir[(j%2)*3] * r;
		pat[npat*2+1] = ddir[(j%2)*3+2] * r;
		// last1 和 last2 分别指向 pat 数组中的不同采样点
	...
	// Start sampling.
	// 开始采样
	// 通过最大速度和速度偏差因子，计算出速度的衰减系数
	float cr = vmax * (1.0f - m_params.velBias);
	float res[3];
	// 初始化作为速度结果值的 res，根据期望速度和速度偏差因子，得到水平方向上的速度分量
	...
	for (int k = 0; k < depth; ++k)
	{
		...
		{
			// 根据当前采样点和速度衰减系数计算新的候选速度。
			float vcand[3];
			vcand[0] = res[0] + pat[i*2+0]*cr;
			vcand[1] = 0;
			vcand[2] = res[2] + pat[i*2+1]*cr;
			
			// 检查新的速度候选项是否超过最大速度的平方。如果超过，则忽略该候选项，继续下一个采样点。
