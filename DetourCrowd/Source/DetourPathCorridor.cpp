...
// path 当前路径数组
// npath 当前路径数组中包含的路径数量
// maxPath 最大路径数量
// visited 已访问点数组
// nvisited 已访问数组中的元素数量
...
// 这个函数用于在路径内规划本地移动。为了规划移动，可以检测到一个或多个角点。它的功能基本上与 dtNavMeshQuery::findStraightPath 相同。
//
// 由于内部优化的原因，返回的角点数量最多为 maxCorners - 1. 例如：如果缓冲区大小为10个角点，该函数最多只会返回9个角点。因此如果需要10
// 个角点，缓冲区应该大小为11个角点。
//
// 如果目标在范围内，它将称为最后一个角点，并且具有多边形引用 ID 为零。
...
/*
* 
* 当代理的位置受到不准确的运动或动态障碍物避免的影响时，可能会导致代理位置明显偏离原始路径。随着时间的推移，这可能导致形成非最优的路径。
* 在瓦片的角落附近也可能形成非最优路径。
* 
* 该函数的目的是在当前位置和目标位置之间优化路径，仅当目标位置从当前位置可见且直接朝向目标位置比沿着现有路径更好时，路径才会发生变化。
* 
* 该函数对于代理的运动越不准确，效果越明显。只需根据代理的需要调整调用的频率即可。
* 
* 需要注意的是，该函数不适用于长距离搜索。
* 
*/
										  ...
										  dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	...
	// 将射线的长度限制在最大距离之内。
	// Clamp 有压制的含义，因此，这里翻译为限制。
	float goal[3];
	dtVcopy(goal, next);
	float dist = dtVdist2D(m_pos, goal);
	
	// If too close to the goal, do not try to optimize.
	// 如果距离目标点太近，则不尝试进行优化处理。
	if (dist < 0.01f)
		return;
	
	// Overshoot a little. This helps to optimize open fields in tiled meshes.
	// 对于开放区域，可以稍微超出目标点的位置进行优化，这有助于在导航网格中优化开放开放区域。
...
// 当代理的运动或动态避障不准确时，可能会导致代理的位置明显偏离原始路径。
// 随着时间的推移，这可能会导致形成非最优的路径。
// 这个函数将使用局部区域搜索来尝试重新优化路径。

// 代理运动越不准确，这个函数的效果就越明显。可以根据代理的需求调整调用频率，以匹配代理的实际情况。
											   ...
											   dtNavMeshQuery* navquery)
{
	...
	// Prune path
	// 删除路径，实际上就是内存拷贝，用后面的内存覆盖掉前面的内存
	// 然后，减少路径的偏移量（表示当前存储路径的计数）
...
* 行为：
* 
*	移动受限于导航网格的表面：代理只能在导航网格上移动，确保其在可行走区域内移动。
*	
*	路径会自动调整（缩短或延迟，以达到路径有效的目的）：为了保持有效性，路径会根据移动过程中的变化进行自动调整。这可能包括路径的缩短
*	或延长，以适应导航路径的变化或障碍物的出现或消失。
* 
*	新位置将位于调整后的路径的第一个多边形上：移动完成后，代理的新位置将位于调整后的路径的第一个多边形上。
* 
* 该功能的预期使用情况是，目标位置与当前路径的位置是相近的。何为 “相近” 取决于局部多边形密度、查询搜索的半径等因素。
* 
* 需要注意的是，最终的位置可能与目标位置不同，如果目标位置不在导航网格上，或者无法通过局部搜索到达目标位置，则会找到一个可到达的位置，
* 即使与目标位置略有差异。这确保了代理的移动在导航网格内，并能够在合理的范围内接近目标位置。
* 
*/
bool dtPathCorridor::movePosition(const float* npos, dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	...
	if (dtStatusSucceed(status)) {
		...
		// 调整位置以保证代理在导航网格上。意思就是，确保代理不会穿过网格或位于障碍物之上
		// 代理始终在正确的、有效的网格上。
		float h = m_pos[1];
...
/// 
/// 当前走廊位置应该位于路径的第一个多边形内。而目标位置应该位于最后一个多边形内。
/// 
/// 警告：路径的大小不能超过在初始化函数 init 中设置的走廊路径缓冲区的大小。
